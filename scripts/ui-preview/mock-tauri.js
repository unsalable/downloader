...
(() => {
  ...
  /** The one cut the Trim screen knows about, and its fake progress. */
  const trim = { status: 'idle', percent: null, outputPath: null, error: null, timer: null };
  const publishTrim = (over) => {
    Object.assign(trim, over);
    return { status: trim.status, percent: trim.percent, outputPath: trim.outputPath, error: null };
  };
  ...
  const commands = {
    ...
    probe_media: ({ path }) => ({
      ...
      // Matches PREVIEW_CLIP below, so the Trim timeline and the picture
      // sitting above it are describing the same thing.
      durationSec: 20.4,
      width: 1920,
      height: 1080,
      fps: 30,
    ...
    trim_state: () => publishTrim({}),
    allow_media_preview: () => null,
    cancel_trim: () => {
      clearInterval(trim.timer);
      trim.timer = null;
      emit('trim://changed', publishTrim({ status: 'canceled', percent: null, outputPath: null }));
      return null;
    },
    start_trim: ({ request }) => {
      clearInterval(trim.timer);
      emit('trim://changed', publishTrim({ status: 'running', percent: 0, outputPath: null }));
      let percent = 0;
      trim.timer = setInterval(() => {
        percent += 9;
        if (percent < 100) {
          emit('trim://changed', publishTrim({ status: 'running', percent }));
          return;
        }
        clearInterval(trim.timer);
        trim.timer = null;
        const name = request.inputPath.split(/[\\/]/).pop().replace(/\.[^.]+$/, '');
        emit('trim://changed', publishTrim({
          status: 'completed',
          percent: 100,
          outputPath: 'C:\\Users\\melik\\Videos\\' + name + ' 0m04.0s-0m15.4s.mp4',
        }));
      }, 260);
      return null;
    },
